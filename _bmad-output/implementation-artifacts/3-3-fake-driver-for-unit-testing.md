...
1. **AC1 — Programmable responses:** `pkg/drivers/fake/driver.go` implements all 9 `StorageProvider` methods. Each method can have its response pre-programmed: `fake.OnPromoteVolume(vgID).Return(nil)` or `fake.OnPromoteVolume(vgID).Return(drivers.ErrPromotionFailed)`. Compile-time interface check: `var _ drivers.StorageProvider = (*Driver)(nil)`.

2. **AC2 — Call recording:** Every method invocation is recorded with the method name and arguments. Callers can assert on recorded calls: `fake.Calls()` returns all calls, `fake.CallsTo("PromoteVolume")` returns calls to a specific method, `fake.CallCount("PromoteVolume")` returns the count.
...
4. **AC4 — Sensible defaults:** When a method is called with no programmed response, it returns success with zero-value results: `CreateVolumeGroup` returns an auto-generated `VolumeGroupID` (`"fake-<uuid>"`), `GetVolumeGroup` returns a zero-value `VolumeGroupInfo`, `GetReplicationInfo` returns a zero-value `ReplicationInfo`, all other methods return `nil` error.
...
- [ ] Task 1: Define the Driver struct and supporting types (AC: #1, #2, #5)
  ...
  - [ ] 1.4 Define `Response` struct (exported, test code may construct directly): `VolumeGroupID drivers.VolumeGroupID`, `VolumeGroupInfo *drivers.VolumeGroupInfo`, `ReplicationInfo *drivers.ReplicationInfo`, `Err error`
...
- [ ] Task 2: Implement response programming API (AC: #1, #3)
  ...
  - [ ] 2.2 Implement typed `On<Method>` methods on `Driver` for all 9 StorageProvider methods. Each accepts an optional `vgID drivers.VolumeGroupID` argument (variadic) for argument matching. Returns `*CallStub`. Methods: `OnCreateVolumeGroup()`, `OnDeleteVolumeGroup(vgID ...drivers.VolumeGroupID)`, `OnGetVolumeGroup(vgID ...drivers.VolumeGroupID)`, `OnEnableReplication(vgID ...drivers.VolumeGroupID)`, `OnDisableReplication(vgID ...drivers.VolumeGroupID)`, `OnPromoteVolume(vgID ...drivers.VolumeGroupID)`, `OnDemoteVolume(vgID ...drivers.VolumeGroupID)`, `OnResyncVolume(vgID ...drivers.VolumeGroupID)`, `OnGetReplicationInfo(vgID ...drivers.VolumeGroupID)`
...
- [ ] Task 3: Implement all 9 StorageProvider methods (AC: #1, #2, #3, #4)
  ...
  - [ ] 3.2 `CreateVolumeGroup(ctx, spec)` — record call, return programmed `VolumeGroupID` + error, or default `"fake-<uuid>"` + nil
  ...
  - [ ] 3.5 `EnableReplication(ctx, vgID)` — record call, return programmed error or nil
  - [ ] 3.6 `DisableReplication(ctx, vgID)` — record call, return programmed error or nil
  - [ ] 3.7 `PromoteVolume(ctx, vgID, opts)` — record call (include opts in args), return programmed error or nil
  - [ ] 3.8 `DemoteVolume(ctx, vgID, opts)` — record call (include opts in args), return programmed error or nil
  - [ ] 3.9 `ResyncVolume(ctx, vgID)` — record call, return programmed error or nil
  - [ ] 3.10 `GetReplicationInfo(ctx, vgID)` — record call, return programmed `ReplicationInfo` + error, or zero-value `ReplicationInfo` + nil
...
- [ ] Task 6: Unit tests (AC: #7)
  - [ ] 6.1 In `pkg/drivers/fake/driver_test.go`:
    ...
    - [ ] 6.1.2 `TestDriver_DefaultBehavior_ReturnsSuccess` — call all 9 methods with no programmed responses, verify success with zero-value/default results
    - [ ] 6.1.3 `TestDriver_CreateVolumeGroup_DefaultReturnsFakeID` — verify default returns a `"fake-"` prefixed ID
    - [ ] 6.1.4 `TestDriver_OnPromoteVolume_ReturnError` — program `OnPromoteVolume(vgID).Return(drivers.ErrPromotionFailed)`, call PromoteVolume, verify error returned and call recorded
    - [ ] 6.1.5 `TestDriver_OnPromoteVolume_ReturnNil` — program success, verify nil error returned
    - [ ] 6.1.6 `TestDriver_OnGetReplicationInfo_ReturnResult` — program with `ReturnResult(Response{ReplicationInfo: &info, Err: nil})`, verify info returned
    ...
    - [ ] 6.1.10 `TestDriver_CallRecording` — make several calls, verify `Calls()` returns all, `CallsTo("PromoteVolume")` returns filtered, `CallCount` is correct, `Called` returns true/false
    ...
    - [ ] 6.1.13 `TestDriver_ErrorInjection_AllMethods` — program each of the 9 methods with a typed error from `pkg/drivers/errors.go`, verify the correct error is returned via `errors.Is`
    - [ ] 6.1.14 `TestDriver_CallArgs_Recorded` — call PromoteVolume with specific vgID and opts, verify Call.Args contains the exact arguments
...
| State | Stateful in-memory (tracks volume groups, replication states) | Stateless (returns programmed responses) |
...
| `ReplicationInfo`, `ReplicationState` constants | `pkg/drivers/types.go` | GetReplicationInfo return type |
| `PromoteOptions`, `DemoteOptions` | `pkg/drivers/types.go` | PromoteVolume/DemoteVolume parameters, Call.Args recording |
| `ErrVolumeNotFound`, `ErrVolumeGroupNotFound`, `ErrPromotionFailed`, etc. | `pkg/drivers/errors.go` | Error injection in tests |
...
fake.OnPromoteVolume("vg-1").Return(drivers.ErrPromotionFailed)

// Program success
fake.OnPromoteVolume("vg-2").Return(nil)

// Program result + error for value-returning methods
fake.OnGetReplicationInfo("vg-1").ReturnResult(fake.Response{
    ReplicationInfo: &drivers.ReplicationInfo{State: drivers.ReplicationActive},
})
...
// Call the method
err := fake.PromoteVolume(ctx, "vg-1", drivers.PromoteOptions{Force: true})
// err == drivers.ErrPromotionFailed

// Assert on recorded calls
calls := fake.CallsTo("PromoteVolume")
// calls[0].Args == []interface{}{"vg-1", drivers.PromoteOptions{Force: true}}
```
...
```go
fake.OnPromoteVolume("vg-1").Return(nil)                        // first call succeeds
fake.OnPromoteVolume("vg-1").Return(drivers.ErrPromotionFailed) // second call fails
```
...
|------|---------|-------------|
| `pkg/drivers/fake/driver.go` | `Driver` struct, `Call`/`Response`/`CallStub` types, all 9 `StorageProvider` method implementations, `On*` programming API, call recording helpers | **New** |
...
- The no-op driver is STATEFUL (tracks volume groups, replication states) — the fake driver is STATELESS (returns programmed responses)
...
- [Source: _bmad-output/project-context.md#StorageProvider Driver Framework] — 9 methods, idempotency, driver packages at `pkg/drivers/<vendor>/`
