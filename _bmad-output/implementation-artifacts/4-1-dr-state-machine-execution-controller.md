...
- [ ] Task 1: Implement the DR state machine (AC: #1, #2, #3)
  - [ ] 1.1 Create `pkg/engine/statemachine.go` with `ErrInvalidPhaseTransition` typed error that wraps current and requested phases
  - [ ] 1.2 Define `validTransitions` map: `map[string]map[ExecutionMode]string` mapping `(currentPhase, mode) → targetPhase`
  - [ ] 1.3 Implement `Transition(currentPhase string, mode ExecutionMode) (string, error)` — validates and returns the in-progress target phase
  - [ ] 1.4 Define `completionTransitions` map: `map[string]string` mapping in-progress phase → completion phase
  - [ ] 1.5 Implement `CompleteTransition(currentPhase string) (string, error)` — returns the next phase after completion
  - [ ] 1.6 Implement `ValidStartingPhases(mode ExecutionMode) []string` — returns phases that accept the given mode (for webhook error messages)
  - [ ] 1.7 Implement `IsTerminalPhase(phase string) bool` — returns true for `FailedOver`, `DRedSteadyState`, `SteadyState` (steady states where no execution is in progress)
  - [ ] 1.8 Add `doc.go` comment or expand existing `pkg/engine/doc.go` to cover state machine

- [ ] Task 2: State machine unit tests (AC: #10)
  - [ ] 2.1 Create `pkg/engine/statemachine_test.go`
  - [ ] 2.2 Table-driven test: `TestTransition_ValidTransitions` — all 4 valid `(phase, mode)` combinations return expected target phase
  - [ ] 2.3 Table-driven test: `TestTransition_InvalidTransitions` — all invalid `(phase, mode)` combinations return `ErrInvalidPhaseTransition`
  - [ ] 2.4 Test: `TestTransition_InvalidPhase_ReturnsError` — unknown phase string returns error
  - [ ] 2.5 Table-driven test: `TestCompleteTransition_ValidCompletions` — all 3 valid completions return expected phase
  - [ ] 2.6 Test: `TestCompleteTransition_InvalidPhase_ReturnsError` — non-in-progress phase returns error
  - [ ] 2.7 Test: `TestTransition_ErrorMessage_ContainsPhases` — error message includes both current and requested phases
  - [ ] 2.8 Test: `TestValidStartingPhases` — returns correct phases per mode
  - [ ] 2.9 Test: `TestIsTerminalPhase` — returns true/false correctly for all 6 phases

- [ ] Task 3: Implement DRExecution admission webhook (AC: #7)
  - [ ] 3.1 Create `pkg/admission/drexecution_validator.go` with `DRExecutionValidator` struct holding `client.Reader` and `admission.Decoder`
  - [ ] 3.2 Add `+kubebuilder:webhook` marker for `/validate-soteria-io-v1alpha1-drexecution` path, `failurePolicy=Fail`, CREATE only
  - [ ] 3.3 Implement `Handle(ctx, req) admission.Response`: decode DRExecution, validate `.spec.planName` non-empty, validate `.spec.mode` is `planned_migration` or `disaster`
  - [ ] 3.4 Fetch the referenced DRPlan using `client.Reader` — if not found, deny with "DRPlan %q not found"
  - [ ] 3.5 Call `engine.Transition(plan.Status.Phase, exec.Spec.Mode)` — if error, deny with descriptive message including current phase and valid starting phases
  - [ ] 3.6 If all checks pass, allow the request

- [ ] Task 4: Register DRExecution webhook (AC: #7)
  - [ ] 4.1 Add `ValidateDRExecutionPath` constant to `pkg/admission/setup.go`
  - [ ] 4.2 Add `SetupDRExecutionWebhook(mgr ctrl.Manager) error` in `setup.go` — creates `DRExecutionValidator` with `mgr.GetAPIReader()` (uncached) and registers it
  - [ ] 4.3 Wire `SetupDRExecutionWebhook` in `cmd/soteria/main.go` alongside existing webhook setup calls

- [ ] Task 5: Enhance DRExecution validation in `validation.go` (AC: #7)
  - [ ] 5.1 In `pkg/apis/soteria.io/v1alpha1/validation.go`, update `ValidateDRExecution` to check: `.spec.planName` required, `.spec.mode` must be `planned_migration` or `disaster`
  - [ ] 5.2 Add `ValidateDRExecutionUpdate` — DRExecution is immutable once created, reject spec changes
  - [ ] 5.3 Update validation tests in `validation_test.go`

- [ ] Task 6: Implement DRExecution controller reconciliation (AC: #4, #5, #6, #8, #9)
  - [ ] 6.1 In `pkg/controller/drexecution/reconciler.go`, add `Recorder events.EventRecorder` field to `DRExecutionReconciler`
  - [ ] 6.2 Implement `Reconcile`: fetch DRExecution, check if already processed (idempotency via `.status.startTime` non-nil)
  - [ ] 6.3 Fetch the referenced DRPlan — if not found, set `Result=Failed`, add condition `type=Ready, status=False, reason=PlanNotFound`, emit event, return no requeue
  - [ ] 6.4 Call `engine.Transition(plan.Status.Phase, exec.Spec.Mode)` — if error, set `Result=Failed`, add condition `type=Ready, status=False, reason=InvalidPhaseTransition`, emit event, return no requeue
  - [ ] 6.5 On success: update `plan.Status.Phase` to the returned in-progress phase via status subresource
  - [ ] 6.6 Set `exec.Status.StartTime = metav1.Now()`, set initial condition `type=Progressing, status=True, reason=ExecutionStarted`
  - [ ] 6.7 Update DRExecution status via status subresource
  - [ ] 6.8 Emit `FailoverStarted` event on the DRPlan with execution name and mode
  - [ ] 6.9 Return `ctrl.Result{}, nil` — actual engine dispatch happens in Story 4.2+

- [ ] Task 7: Update main.go wiring (AC: #4, #7)
  - [ ] 7.1 Pass `Recorder` to `DRExecutionReconciler` (create dedicated event recorder for drexecution-controller)
  - [ ] 7.2 Call `SetupDRExecutionWebhook(mgr)` after existing webhook setup

- [ ] Task 8: Controller unit tests with envtest (AC: #10)
  - [ ] 8.1 Create DRExecution CRD fixture in `test/integration/controller/suite_test.go` (or extend existing setup)
  - [ ] 8.2 Register DRExecution controller in the test manager alongside DRPlan controller
  - [ ] 8.3 Test: `TestDRExecutionReconciler_SuccessfulSetup` — create DRPlan in SteadyState + DRExecution with planned_migration → plan transitions to FailingOver, execution gets startTime
  - [ ] 8.4 Test: `TestDRExecutionReconciler_PlanNotFound` — execution references nonexistent plan → Result=Failed, no requeue
  - [ ] 8.5 Test: `TestDRExecutionReconciler_InvalidPhase` — DRPlan in FailedOver + planned_migration → Result=Failed, plan phase unchanged
  - [ ] 8.6 Test: `TestDRExecutionReconciler_IdempotentRereconcile` — reconcile same execution twice → no double-transition, no error
  - [ ] 8.7 Test: `TestDRExecutionReconciler_DisasterMode` — DRPlan in SteadyState + disaster → FailingOver

- [ ] Task 9: Admission webhook tests (AC: #10)
  - [ ] 9.1 Create `pkg/admission/drexecution_validator_test.go`
  - [ ] 9.2 Test: valid DRExecution CREATE accepted (plan exists, correct phase)
  - [ ] 9.3 Test: missing `.spec.planName` → denied
  - [ ] 9.4 Test: invalid `.spec.mode` → denied
  - [ ] 9.5 Test: plan not found → denied with descriptive message
  - [ ] 9.6 Test: plan in wrong phase → denied with message explaining valid phases

- [ ] Task 10: Documentation and verification
  - [ ] 10.1 Update `pkg/engine/doc.go` to cover state machine
  - [ ] 10.2 Add/update RBAC markers on DRExecution reconciler for drplans/status update access
  - [ ] 10.3 Run `make manifests` to regenerate RBAC/webhook configs
  - [ ] 10.4 Run `make generate` if types changed
  - [ ] 10.5 Run `make test` — all unit tests pass
  - [ ] 10.6 Run `make lint-fix` followed by `make lint` — no new lint errors
  - [ ] 10.7 Run `make build` — compiles cleanly
