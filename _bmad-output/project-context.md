...
- Unified handler model: `FailoverHandler` (config: `GracefulShutdown bool`) implements both failover and failback. Per-group path is always `SetSource → StartVM` (SetSource promotes target to primary/writable). Step 0 (planned only) runs `StopVM + StopReplication` to demote the source to secondary. `ReprotectHandler` implements both reprotect and restore. Direction is encoded in state machine phases, not handler logic
