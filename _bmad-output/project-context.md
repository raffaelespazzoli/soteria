...
- API group: `soteria.io/v1alpha1` — resources: `drplans`, `drexecutions`
...
- Unified handler model: `FailoverHandler` (config: `GracefulShutdown bool`) implements both failover and failback. Per-group path is always `SetSource → StartVM` (SetSource promotes target to primary/writable). Step 0 (planned only) runs `StopVM → ResyncVolume` then returns `ErrResyncRequested`; the reconciler's event-driven resync gate waits for VR/VGR completion (via watch, not polling) before calling `StopReplication` to demote the source. Configurable timeout via `DRPlanSpec.ResyncTimeout` (default 10m). In multi-site, the target site sets `ResyncComplete` after observing its local VR/VGR completion; the source site then demotes and sets `Step0Complete`. `ReprotectHandler` implements both reprotect and restore. Direction is encoded in state machine phases, not handler logic
...
2. Aggregated API Server with DRPlan/DRExecution types
