...
func TestDRExecutionValidator_RejectWhenDisksInconsistent(t *testing.T) {
	...
	if !strings.Contains(msg, "disk topology does not match") {
		t.Errorf("expected message about disk topology, got %q", msg)
	}
}

func TestDRExecutionValidator_AllowWhenDisksConsistent(t *testing.T) {
	reader := &stubReader{
		plans: map[string]*soteriav1alpha1.DRPlan{
			"my-plan": {
				ObjectMeta: metav1.ObjectMeta{Name: "my-plan"},
	...
	resp := v.Handle(context.Background(), makeExecRequest(exec, admissionv1.Create))
	if !resp.Allowed {
		t.Errorf("expected allowed when DisksConsistent is True, got denied: %v", resp.Result)
	}
}

func TestDRExecutionValidator_RequiresBothSitesInSyncAndDisksConsistent(t *testing.T) {
	reader := &stubReader{
		plans: map[string]*soteriav1alpha1.DRPlan{
			"my-plan": {
				ObjectMeta: metav1.ObjectMeta{Name: "my-plan"},
	...
	resp := v.Handle(context.Background(), makeExecRequest(exec, admissionv1.Create))
	if resp.Allowed {
		t.Error("expected denied when SitesInSync=False (even if DisksConsistent=True)")
	}
	msg := ""
	if resp.Result != nil {
		msg = resp.Result.Message
