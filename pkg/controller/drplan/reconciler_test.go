...
// mockUnprotectedVMDiscoverer implements engine.UnprotectedVMDiscoverer for unit tests.
type mockUnprotectedVMDiscoverer struct {
	vms []engine.VMReference
	err error
}

func (m *mockUnprotectedVMDiscoverer) ListUnprotectedVMs(_ context.Context) ([]engine.VMReference, error) {
	return m.vms, m.err
}

// Compile-time check.
var _ engine.UnprotectedVMDiscoverer = (*mockUnprotectedVMDiscoverer)(nil)

func newReconcilerWithUnprotected(
	objs []client.Object,
	discoverer engine.VMDiscoverer,
	unprotected engine.UnprotectedVMDiscoverer,
) (*DRPlanReconciler, client.Client) {
	scheme := newTestScheme()
	fakeClient := fake.NewClientBuilder().
		WithScheme(scheme).
	...
	return &DRPlanReconciler{
		...
		Recorder:                events.NewFakeRecorder(10),
		UnprotectedVMDiscoverer: unprotected,
	}, fakeClient
}

func TestReconcile_UnprotectedVMCount_PopulatedCorrectly(t *testing.T) {
	tests := []struct {
		name      string
		unprotVMs []engine.VMReference
		wantCount int
	}{
		{
			name:      "zero unprotected VMs",
			unprotVMs: nil,
			wantCount: 0,
		},
		{
			name: "one unprotected VM",
			unprotVMs: []engine.VMReference{
				{Name: "orphan-1", Namespace: "dev"},
			},
			wantCount: 1,
		},
		{
			name: "multiple unprotected VMs",
			unprotVMs: []engine.VMReference{
				{Name: "orphan-1", Namespace: "dev"},
				{Name: "orphan-2", Namespace: "staging"},
				{Name: "orphan-3", Namespace: "prod"},
			},
			wantCount: 3,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			plan := newTestPlan()
			protectedVMs := []engine.VMReference{
				{Name: "vm-1", Namespace: "default", Labels: map[string]string{"soteria.io/wave": "1"}},
			}

			unprotMock := &mockUnprotectedVMDiscoverer{vms: tt.unprotVMs}
			r, c := newReconcilerWithUnprotected([]client.Object{plan}, &mockVMDiscoverer{vms: protectedVMs}, unprotMock)
			...
			if updated.Status.UnprotectedVMCount != tt.wantCount {
				t.Errorf("UnprotectedVMCount = %d, want %d", updated.Status.UnprotectedVMCount, tt.wantCount)
			}
		})
	}
}

func TestReconcile_UnprotectedVMs_PreflightPopulated(t *testing.T) {
	plan := newTestPlan()
	protectedVMs := []engine.VMReference{
		{Name: "vm-1", Namespace: "default", Labels: map[string]string{"soteria.io/wave": "1"}},
	}
	unprotectedVMs := []engine.VMReference{
		{Name: "orphan-1", Namespace: "dev"},
		{Name: "orphan-2", Namespace: "prod"},
	}

	r, c := newReconcilerWithUnprotected(
		[]client.Object{plan},
		&mockVMDiscoverer{vms: protectedVMs},
		&mockUnprotectedVMDiscoverer{vms: unprotectedVMs},
	)
	...
	if updated.Status.Preflight == nil {
		t.Fatal("Preflight report should be populated")
	}
	if len(updated.Status.Preflight.UnprotectedVMs) != 2 {
		t.Fatalf("Preflight.UnprotectedVMs = %d, want 2", len(updated.Status.Preflight.UnprotectedVMs))
	}
	if updated.Status.Preflight.UnprotectedVMs[0].Name != "orphan-1" {
		t.Errorf("first unprotected VM = %q, want orphan-1", updated.Status.Preflight.UnprotectedVMs[0].Name)
	}
}

func TestReconcile_UnprotectedVMs_TruncatedAt100(t *testing.T) {
	plan := newTestPlan()
	protectedVMs := []engine.VMReference{
		{Name: "vm-1", Namespace: "default", Labels: map[string]string{"soteria.io/wave": "1"}},
	}

	manyVMs := make([]engine.VMReference, 150)
	for i := range manyVMs {
		manyVMs[i] = engine.VMReference{
	...
	r, c := newReconcilerWithUnprotected(
		[]client.Object{plan},
		&mockVMDiscoverer{vms: protectedVMs},
		&mockUnprotectedVMDiscoverer{vms: manyVMs},
	)
	...
	if updated.Status.UnprotectedVMCount != 150 {
		t.Errorf("UnprotectedVMCount = %d, want 150 (full count)", updated.Status.UnprotectedVMCount)
	}
	if updated.Status.Preflight == nil {
		t.Fatal("Preflight report should be populated")
	}
	if len(updated.Status.Preflight.UnprotectedVMs) != 100 {
		t.Errorf("Preflight.UnprotectedVMs = %d, want 100 (truncated)", len(updated.Status.Preflight.UnprotectedVMs))
	}

	hasTruncationWarning := false
	for _, w := range updated.Status.Preflight.Warnings {
		if contains(w, "Showing 100 of 150") {
			hasTruncationWarning = true
			break
		}
	}
	if !hasTruncationWarning {
		t.Errorf("Expected truncation warning, got: %v", updated.Status.Preflight.Warnings)
	}
}

func TestReconcile_UnprotectedVMs_EventTransitions(t *testing.T) {
	plan := newTestPlan()
	protectedVMs := []engine.VMReference{
		{Name: "vm-1", Namespace: "default", Labels: map[string]string{"soteria.io/wave": "1"}},
	}

	unprotMock := &mockUnprotectedVMDiscoverer{vms: nil}
	r, c := newReconcilerWithUnprotected([]client.Object{plan}, &mockVMDiscoverer{vms: protectedVMs}, unprotMock)
	recorder := r.Recorder.(*events.FakeRecorder)

	// First reconcile — ObservedGeneration == 0, no unprotected event expected.
	_, err := r.Reconcile(context.Background(), ctrl.Request{
		NamespacedName: types.NamespacedName{Name: "plan-1"},
	})
	if err != nil {
	...
	for _, ev := range firstEvents {
		if contains(ev, "UnprotectedVMsDetected") || contains(ev, "AllVMsProtected") {
			t.Errorf("First reconcile should not emit unprotected events, got: %s", ev)
		}
	}

	// Second reconcile: 0→5 transition — Warning event expected.
	unprotMock.vms = make([]engine.VMReference, 5)
	for i := range unprotMock.vms {
		unprotMock.vms[i] = engine.VMReference{Name: fmt.Sprintf("orphan-%d", i), Namespace: "dev"}
	}
	_, err = r.Reconcile(context.Background(), ctrl.Request{
		NamespacedName: types.NamespacedName{Name: "plan-1"},
	})
	...
	if updated.Status.UnprotectedVMCount != 5 {
		t.Errorf("UnprotectedVMCount = %d, want 5", updated.Status.UnprotectedVMCount)
	}
	secondEvents := drainEvents(recorder)
	if !hasEventWithReason(secondEvents, "UnprotectedVMsDetected") {
		t.Errorf("Expected UnprotectedVMsDetected event on 0→5 transition, got: %v", secondEvents)
	}

	// Third reconcile: 5→3 (still unprotected, no transition event).
	unprotMock.vms = unprotMock.vms[:3]
	_, err = r.Reconcile(context.Background(), ctrl.Request{
		NamespacedName: types.NamespacedName{Name: "plan-1"},
	})
	if err != nil {
	...
	if updated.Status.UnprotectedVMCount != 3 {
		t.Errorf("UnprotectedVMCount = %d, want 3", updated.Status.UnprotectedVMCount)
	}
	thirdEvents := drainEvents(recorder)
	for _, ev := range thirdEvents {
		if contains(ev, "UnprotectedVMsDetected") || contains(ev, "AllVMsProtected") {
			t.Errorf("N→N transition should not emit unprotected events, got: %s", ev)
		}
	}

	// Fourth reconcile: 3→0 transition — Normal event expected.
	unprotMock.vms = nil
	_, err = r.Reconcile(context.Background(), ctrl.Request{
		NamespacedName: types.NamespacedName{Name: "plan-1"},
	})
	...
	if updated.Status.UnprotectedVMCount != 0 {
		t.Errorf("UnprotectedVMCount = %d, want 0", updated.Status.UnprotectedVMCount)
	}
	fourthEvents := drainEvents(recorder)
	if !hasEventWithReason(fourthEvents, "AllVMsProtected") {
		t.Errorf("Expected AllVMsProtected event on 3→0 transition, got: %v", fourthEvents)
	}
}
...
func hasEventWithReason(evts []string, reason string) bool {
	for _, ev := range evts {
...
func TestReconcile_UnprotectedVMs_ErrorDoesNotBlockReconcile(t *testing.T) {
	plan := newTestPlan()
	protectedVMs := []engine.VMReference{
		{Name: "vm-1", Namespace: "default", Labels: map[string]string{"soteria.io/wave": "1"}},
	}

	unprotMock := &mockUnprotectedVMDiscoverer{err: fmt.Errorf("kubevirt API unavailable")}
	r, c := newReconcilerWithUnprotected([]client.Object{plan}, &mockVMDiscoverer{vms: protectedVMs}, unprotMock)
	...
	if err != nil {
		t.Fatalf("Reconcile() should not error when ListUnprotectedVMs fails, got: %v", err)
	}
	...
	readyCond := findReadyCondition(updated.Status.Conditions)
	if readyCond == nil || readyCond.Status != metav1.ConditionTrue {
		t.Error("Ready condition should be True even when ListUnprotectedVMs fails")
	}
}

func TestReconcile_UnprotectedVMs_NilDiscoverer_NoCrash(t *testing.T) {
	plan := newTestPlan()
	protectedVMs := []engine.VMReference{
		{Name: "vm-1", Namespace: "default", Labels: map[string]string{"soteria.io/wave": "1"}},
	}

	r, c := newReconciler([]client.Object{plan}, &mockVMDiscoverer{vms: protectedVMs})

	_, err := r.Reconcile(context.Background(), ctrl.Request{
		NamespacedName: types.NamespacedName{Name: "plan-1"},
	})
	if err != nil {
		t.Fatalf("Reconcile() should not error with nil UnprotectedVMDiscoverer, got: %v", err)
	}
	...
	if updated.Status.UnprotectedVMCount != 0 {
		t.Errorf("UnprotectedVMCount = %d, want 0 (no discoverer set)", updated.Status.UnprotectedVMCount)
	}

	readyCond := findReadyCondition(updated.Status.Conditions)
	if readyCond == nil || readyCond.Status != metav1.ConditionTrue {
		t.Error("Ready condition should be True with nil UnprotectedVMDiscoverer")
	}
}
