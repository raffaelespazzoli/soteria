...
func TestReplication_ResourceCreatedOnDC1_VisibleOnDC2(t *testing.T) {
	...
	plan := newDRPlan("repl-dc1-to-dc2", "default")
	out := &v1alpha1.DRPlan{}
	ctx := context.Background()

	if err := dc1Store.Create(ctx, planKey("default", plan.Name), plan, out, 0); err != nil {
		t.Fatalf("Create on DC1 failed: %v", err)
	}

	// Poll DC2 until the resource appears (async replication window).
	got := &v1alpha1.DRPlan{}
	if err := pollUntilFound(t, dc2Store, planKey("default", plan.Name), got, 15*time.Second); err != nil {
...
func TestReplication_ResourceCreatedOnDC2_VisibleOnDC1(t *testing.T) {
	...
	plan := newDRPlan("repl-dc2-to-dc1", "default")
	out := &v1alpha1.DRPlan{}
	ctx := context.Background()

	if err := dc2Store.Create(ctx, planKey("default", plan.Name), plan, out, 0); err != nil {
		t.Fatalf("Create on DC2 failed: %v", err)
	}

	got := &v1alpha1.DRPlan{}
	if err := pollUntilFound(t, dc1Store, planKey("default", plan.Name), got, 15*time.Second); err != nil {
...
func TestReplication_MultipleResources_AllReplicateWithinWindow(t *testing.T) {
	...
	for i := range count {
		...
		names[i] = name
		plan := newDRPlan(name, "batch-ns")
		out := &v1alpha1.DRPlan{}
		if err := dc1Store.Create(ctx, planKey("batch-ns", name), plan, out, 0); err != nil {
	...
	for _, name := range names {
		...
		if err := pollUntilFound(t, dc2Store, planKey("batch-ns", name), got, remaining); err != nil {
...
func TestResilience_DC1Down_DC2ContinuesReads(t *testing.T) {
	...
	plan := newDRPlan("resil-reads", "default")
	out := &v1alpha1.DRPlan{}
	if err := dc1Store.Create(ctx, planKey("default", plan.Name), plan, out, 0); err != nil {
	...
	if err := dc2Store.Get(ctx, planKey("default", plan.Name), storage.GetOptions{}, got2); err != nil {
...
func TestResilience_DC1Down_DC2ContinuesWrites(t *testing.T) {
	...
	exec := newDRExecution("resil-write", "default")
	out := &v1alpha1.DRExecution{}
	if err := dc2Store.Create(ctx, execKey("default", exec.Name), exec, out, 0); err != nil {
...
func TestResilience_DC1Down_DC2NoErrors(t *testing.T) {
	...
	plan := newDRPlan("resil-no-errors", "default")
	out := &v1alpha1.DRPlan{}
	...
	for i := range 5 {
		...
		p := newDRPlan(name, "default")
		o := &v1alpha1.DRPlan{}
		if err := dc2Store.Create(ctx, planKey("default", name), p, o, 0); err != nil {
...
func TestResilience_DC2Down_DC1ContinuesOperations(t *testing.T) {
	...
	plan := newDRPlan("resil-dc2-down", "default")
	out := &v1alpha1.DRPlan{}
	if err := dc1Store.Create(ctx, planKey("default", plan.Name), plan, out, 0); err != nil {
	...
	got := &v1alpha1.DRPlan{}
	if err := dc1Store.Get(ctx, planKey("default", plan.Name), storage.GetOptions{}, got); err != nil {
...
func TestRecovery_DC1Recovers_StateReconciles(t *testing.T) {
	...
	for i := range count {
		...
		names[i] = name
		plan := newDRPlan(name, "recovery-ns")
		out := &v1alpha1.DRPlan{}
		if err := dc2Store.Create(ctx, planKey("recovery-ns", name), plan, out, 0); err != nil {
	...
	for _, name := range names {
		got := &v1alpha1.DRPlan{}
		if err := pollUntilFound(t, dc1Store, planKey("recovery-ns", name), got, 30*time.Second); err != nil {
...
func TestRecovery_BothDCsIdenticalAfterReconciliation(t *testing.T) {
	...
	planA := newDRPlan("identical-a", "identical-ns")
	planB := newDRPlan("identical-b", "identical-ns")
	outA := &v1alpha1.DRPlan{}
	outB := &v1alpha1.DRPlan{}

	if err := dc1Store.Create(ctx, planKey("identical-ns", planA.Name), planA, outA, 0); err != nil {
		t.Fatalf("Create A on DC1: %v", err)
	}
	if err := dc2Store.Create(ctx, planKey("identical-ns", planB.Name), planB, outB, 0); err != nil {
	...
	if err := dc1NSStore.GetList(ctx, "/soteria.io/drplans/identical-ns/", listOpts, dc1List); err != nil {
		t.Fatalf("List DC1: %v", err)
	}
	if err := dc2NSStore.GetList(ctx, "/soteria.io/drplans/identical-ns/", listOpts, dc2List); err != nil {
...
func TestRecovery_NoManualIntervention(t *testing.T) {
	...
	plan := newDRPlan("no-manual", "auto-ns")
	out := &v1alpha1.DRPlan{}
	if err := dc2Store.Create(ctx, planKey("auto-ns", plan.Name), plan, out, 0); err != nil {
	...
	dc1Store := newDRPlanStoreForDC(dc1Session)
	got := &v1alpha1.DRPlan{}
	if err := pollUntilFound(t, dc1Store, planKey("auto-ns", plan.Name), got, 15*time.Second); err != nil {
...
func TestConflict_ConcurrentNonCriticalWrite_LastWriteWins(t *testing.T) {
	...
	plan := newDRPlan("lww-test", "conflict-ns")
	out := &v1alpha1.DRPlan{}
	if err := dc1Store.Create(ctx, planKey("conflict-ns", plan.Name), plan, out, 0); err != nil {
		t.Fatalf("Create: %v", err)
	}

	// Wait for replication.
	got := &v1alpha1.DRPlan{}
	if err := pollUntilFound(t, dc2Store, planKey("conflict-ns", plan.Name), got, 15*time.Second); err != nil {
	...
	go func() {
		defer wg.Done()
		_ = updateLabel(dc1Store, ctx, plan.Name, "conflict-ns", "source", "dc1")
	}()
	go func() {
		defer wg.Done()
		_ = updateLabel(dc2Store, ctx, plan.Name, "conflict-ns", "source", "dc2")
	}()
	wg.Wait()
	...
	if err := dc1Store.Get(ctx, planKey("conflict-ns", plan.Name), storage.GetOptions{}, final); err != nil {
...
func TestConflict_ConcurrentPhaseTransition_LWTPreventsDuplicate(t *testing.T) {
	...
	plan := newDRPlanWithPhase("lwt-phase", "conflict-ns", v1alpha1.PhaseSteadyState)
	out := &v1alpha1.DRPlan{}
	if err := dc1Store.Create(ctx, planKey("conflict-ns", plan.Name), plan, out, 0); err != nil {
		t.Fatalf("Create: %v", err)
	}

	if err := pollUntilFound(t, dc2Store, planKey("conflict-ns", plan.Name), &v1alpha1.DRPlan{}, 15*time.Second); err != nil {
		t.Fatalf("Pre-replication: %v", err)
	}
	...
	go func() {
		defer wg.Done()
		dc1Err = updatePhase(dc1Store, ctx, plan.Name, "conflict-ns", v1alpha1.PhaseFailingOver)
	}()
	go func() {
		defer wg.Done()
		dc2Err = updatePhase(dc2Store, ctx, plan.Name, "conflict-ns", v1alpha1.PhaseFailingOver)
	}()
	wg.Wait()
...
func TestConflict_LWT_StalePhaseRejected(t *testing.T) {
	...
	plan := newDRPlanWithPhase("stale-phase", "conflict-ns", v1alpha1.PhaseSteadyState)
	out := &v1alpha1.DRPlan{}
	if err := store.Create(ctx, planKey("conflict-ns", plan.Name), plan, out, 0); err != nil {
		t.Fatalf("Create: %v", err)
	}

	// First update succeeds.
	if err := updatePhase(store, ctx, plan.Name, "conflict-ns", v1alpha1.PhaseFailingOver); err != nil {
		t.Fatalf("First phase update: %v", err)
	}

	// Second update with a different phase should also succeed (CAS on resourceVersion).
	if err := updatePhase(store, ctx, plan.Name, "conflict-ns", v1alpha1.PhaseFailedOver); err != nil {
		t.Fatalf("Second phase update: %v", err)
	}

	// Verify final state.
	got := &v1alpha1.DRPlan{}
	if err := store.Get(ctx, planKey("conflict-ns", plan.Name), storage.GetOptions{}, got); err != nil {
		t.Fatalf("Get: %v", err)
	}
...
func TestConflict_LWT_TableDriven(t *testing.T) {
	...
	for i, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			plan := newDRPlanWithPhase(name, "table-ns", tc.initialPhase)
			out := &v1alpha1.DRPlan{}
			if err := store.Create(ctx, planKey("table-ns", name), plan, out, 0); err != nil {
				t.Fatalf("Create: %v", err)
			}

			err := updatePhase(store, ctx, name, "table-ns", tc.updatePhase)
...
func planKey(ns, name string) string {
	return fmt.Sprintf("/soteria.io/drplans/%s/%s", ns, name)
}

func execKey(ns, name string) string {
	return fmt.Sprintf("/soteria.io/drexecutions/%s/%s", ns, name)
}

func newDRPlan(name, namespace string) *v1alpha1.DRPlan {
...
func newDRPlanWithPhase(name, namespace, phase string) *v1alpha1.DRPlan {
	p := newDRPlan(name, namespace)
	p.Status.Phase = phase
	return p
}

func newDRExecution(name, namespace string) *v1alpha1.DRExecution {
...
func updateLabel(store *scyllastore.Store, ctx context.Context, name, ns, key, value string) error {
	return store.GuaranteedUpdate(ctx, planKey(ns, name), &v1alpha1.DRPlan{}, false,
...
func updatePhase(store *scyllastore.Store, ctx context.Context, name, ns, phase string) error {
	return store.GuaranteedUpdate(ctx, planKey(ns, name), &v1alpha1.DRPlan{}, false,
		nil, func(existing runtime.Object, _ storage.ResponseMeta) (runtime.Object, *uint64, error) {
			plan := existing.(*v1alpha1.DRPlan)
